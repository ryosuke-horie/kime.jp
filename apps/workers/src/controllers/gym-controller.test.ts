...
describe("GymController - 単体テスト", () => {
	...
	describe("getGyms", () => {
		it("デフォルトのクエリパラメータでジム一覧を返すこと", async () => {
			...
			await controller.getGyms(mockCtx as any);
			
			// サービスが正しいパラメータで呼び出されたか検証
			expect(mockService.getGyms).toHaveBeenCalledWith(expect.objectContaining({
				page: 1,
				limit: 10,
				sort: "-createdAt",
			}));
			...
			const calledWith = mockCtx.json.mock.calls[0][0];
		...
		it("サービスでエラーが発生した場合に500エラーを返すこと", async () => {
			...
			expect(mockCtx.json).toHaveBeenCalled();
			const calledWith = mockCtx.json.mock.calls[0][0];
			const options = mockCtx.json.mock.calls[0][1];
	...
	describe("getGymById", () => {
		it("存在するジムIDで正しいジム情報を返すこと", async () => {
			...
			await controller.getGymById(mockCtx as any);
		...
		it("存在しないジムIDでは404エラーを返すこと", async () => {
			...
			// サービスがエラーをスローするようにモック
			mockService.getGymById = vi.fn().mockRejectedValue(
				new Error("Gym with ID non-existent not found")
			);
	...
	describe("createGym", () => {
		it("有効なデータでジムを作成すること", async () => {
			...
			await controller.createGym(mockCtx as any);
	...
	describe("updateGym", () => {
		it("有効なデータでジムを更新すること", async () => {
			...
			await controller.updateGym(mockCtx as any);
		...
		it("存在しないジムIDでは404エラーを返すこと", async () => {
			...
			// サービスがエラーをスローするようにモック
			mockService.updateGym = vi.fn().mockRejectedValue(
				new Error("Gym with ID non-existent not found")
			);
	...
	describe("deleteGym", () => {
		it("存在するジムIDでジムを削除すること", async () => {
			...
			await controller.deleteGym(mockCtx as any);
		...
		it("存在しないジムIDでは404エラーを返すこと", async () => {
			...
			// サービスがエラーをスローするようにモック
			mockService.deleteGym = vi.fn().mockRejectedValue(
				new Error("Gym with ID non-existent not found")
			);
