...
import { Env } from "../../../env";
import { getDatabaseClient, bookClass } from "../../../lib/do-client";
...
bookingRouter.post("/", async (c) => {
  ...
  try {
    ...
    if (result.error) {
      ...
      } else if (result.error.includes("already has a booking")) {
        return c.json({ error: "既にこのクラスに予約があります", bookingId: result.bookingId }, 409);
