...
export function getDatabaseClient(env: Env) {
  ...
  return {
    ...
    update: (table: string, id: string, data: Record<string, unknown>) => db.update(table, id, data),
    delete: (table: string, id: string) => db.delete(table, id),
    query: async <T = unknown>(sql: string): Promise<{ success: boolean; data?: T; error?: string }> => {
      try {
