...
export class Database {
  ...
  async update(
    ...
    try {
      ...
      switch (table) {
        case "gyms":
          await this.db.update(schema.gyms).set(data as any).where(eq(schema.gyms.gymId, id));
          break;
        case "members":
          await this.db.update(schema.members).set(data as any).where(eq(schema.members.memberId, id));
          break;
        case "classes":
          await this.db.update(schema.classes).set(data as any).where(eq(schema.classes.classId, id));
          break;
        case "bookings":
          await this.db.update(schema.bookings).set(data as any).where(eq(schema.bookings.bookingId, id));
          break;
        // 他のテーブルも同様に追加可能
        default:
          return { success: false, error: "Unknown table" };
